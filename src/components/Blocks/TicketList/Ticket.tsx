...
// Hero (single) — desktop numeric "31 / 07 / 2026".
const numericDate = new Intl.DateTimeFormat('en-GB', {
  day: '2-digit',
  month: '2-digit',
  year: 'numeric',
  timeZone: TZ,
...
const Ticket = ({ ticket }: TicketProps) => {
  ...
  const desktopDate = date
    ? numericDate.format(date).replace(/\//g, ' / ')
    : null;
  ...
  return (
    <div className='lg:flex lg:items-stretch'>
      {/* Mobile: the entire card acts as the buy button (no separate button) */}
      <BuyTicketCard
        ticketId={ticket._id}
        disabled={soldOut}
        className='lg:hidden'
