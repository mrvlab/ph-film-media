...
export async function POST(request: Request) {
  ...
  const ticket = await client.fetch<TicketSummary | null>(
    `*[_type == "ticket" && _id == $id][0]{
      _id, title, date, price, currency, totalSeats, seatsSold, poster
    }`,
    { id: ticketId }
  );
  ...
  const posterUrl = ticket.poster?.asset?._ref
    ? urlFor(ticket.poster).width(800).fit('max').url()
    : undefined;

  const showDescription = ticket.date
  ...
  try {
    session = await stripe.checkout.sessions.create({
      ...
      cancel_url: `${origin}/tickets-test`,
    });
  } catch (err) {
